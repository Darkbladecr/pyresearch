...
def calc_authorData(query, records, authorName):
	...
	for record in records:
		if query in record[authorName]:
			count += 1
			idlist[query]['articles'].append({'PMID': record['PMID'], 'Citations': int(record['Citations'])})
	idlist[query]['articles'] = sorted(idlist[query]['articles'], key=itemgetter('Citations'), reverse=True)
