#!/bin/sh
QUERY="stereotactic AND (radiotherapy OR radiosurgery) OR radiosurgery"
DATA=$1
...
python pyresearch_subsection.py -s "$QUERY AND (Ateriovenous malformation OR AVM)" -i $DATA -t "AVM"
